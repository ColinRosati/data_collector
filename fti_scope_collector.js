...
// const path = ('/home/pi/data_collector_access_point/scope_data/scopedata.txt')
const path = ('./scope_data/scopedata.txt')
const locPath = ("./scope_data/scopedata.txt"); //buffer scope file
// const scope_index = ("/home/pi/data_collector_access_point/scope_data/scopedata_index.json"); // JSON formated index file
...
// ---------- testing this start time from scope_collection.js
// var start = new Date()
...
class index
{
	...
	index_writer(lineCount, start , hrstart){
		const data = formBody
		var self = this
		if(lineCount==0){
	    ...
	    console.log("file size", filesize.size/ 1000000.0, "megabytes") //stats.size / 1000000.0 for megabytes 
		...
		new Promise(function(resolve,reject){
			...
			result(setTimeout(function(){
				console.log("index_writer about to call stream_end_check")
				const scope_data = new Fti_scope.scope_collector
				// scope_data.rpc_stream_end()
				scope_data.stream_end_check()
...
var scope_trigger = async(data, callback, dsp)=>{
	if(data){
		...
		let _lines = async(lines)=>{
			...
			const scope_data = new Fti_scope.scope_collector(dsp)
				// console.log(command, formTime)

...
class FtiRpc{
	...
	rpc1(func,args,string,timeout,callBack){
		// console.log('rpc1 this.port', this.port)
		var payload = this.payloadForRpc(func,args,string);
		var packet = this.frame(payload);
		console.log(func,args,string,timeout,callBack)
		// console.log('rpc1 packet', packet,'port', this.port)
		this.port.write(packet);
		this.port.callBack = callBack
	...
	rpc0(func,args,string,callBack){
		var payload = this.payloadForRpc(func,args,string);
		var packet = this.frame(payload);
		console.log('rpc0 payload',payload)
		console.log('rpc0 packet',packet)
		this.port.write(packet);
...
//			console.log('no callBack set')
	...
	payloadForRpc(func,args,string){
		...
		if(string){
			//	string = Buffer.from(string); not going to deal with this yet
		}
...
class FtiRpcUdpSocket{
	constructor(host,port){
		...
		this.socket.bind(0,'0.0.0.0', function(){
			// console.log('socket bound from rpc.js')
		});
		var self = this;

		this.socket.on('message',function(e, rinfo){
			// console.log('e, rinfo rpc.js socket', e, rinfo)

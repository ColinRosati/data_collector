...
LocatorClient.prototype ={
	...
	discover_query: function() {
		...
		return Buffer.from(Array.prototype.concat.apply([],pkt));
...
class ArmLocator{
	...
	static scan(secTimeout, callBack, addr){
		console.log("arloc scan")
		var list = NetInterface.find(/^Ethernet|^en|^eth/);
		console.log("arm locator", list);
		var devlist=[];
		var listeners = [];
		var senders = [];
		var dev , message = '' ;
		...
		new Promise((resolve,reject)=>{
				list.forEach(function(nif,i){
					...
					// setTimeout(function(){
						var scope_data = dev.extract_loc_data(message);
						callBack(scope_data);
					// }, 20);	
				})	
		return
...
*/
	...
	// var dsp = '';
	// if(data.readUInt8(47) == 1){
	// 	console.log("arloc this is a dsp",data.readUInt8(47))
	// }
	switch(data.readUInt8(47)){
		case 1:
		...
		default:
			...
			var name = this.nm
			var parsed_data = this.parse_name(loc);
			console.log('this is not a dspboard')
			var extracted_data = [this.name,parsed_data[1],this.ip, 'not a dspboard'];
			return extracted_data;
	}

	// var loc_dsp = BufferPack.unpack((data.length - 47) + 'H', data, 47);
	...
	// var name = this.nm
	// var parsed_data = this.parse_name(loc);
	// console.log('this is the parsed data looking for translated fti app = ',conf)
	// var extracted_data = [this.name,parsed_data[1],this.ip];
	// return extracted_data;
	}
	...
	parse_loc_data(data){
		...
		this.name = (Buffer.from(data.slice(6,18))).toString().trim();
		this.s_key = data.slice(18,34);
		this.app_name = (Buffer.from(data.slice(34,-1))).toString().trim()
	}
